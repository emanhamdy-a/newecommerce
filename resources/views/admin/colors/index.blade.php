...
<div class="box">
  ...
  <div class="box-body maxContainer allElements"
    ...
    <table class='filter'>
      <tr>
        <td>
          <a class='color_create btn btn-info' href="/admin/colors/create">+ {{trans('admin.add')}}</i></a>
        </td>
        <td>
      ...
      <table id="datatable" style='width:100% !important;'
        ...
        <tbody>
          <?php foreach ($colors as $color) { ?>
            <tr class='container{{ $color->id }}'>
              <td> <input type="checkbox" name="item[]" class="item_checkbox" value="{{ $color->id }}"> </td>
              <td>{{$color->id}}</td>
              <td>
                <?php //if(lang()=='ar'){ ?>
                  {{lang()=='ar' ? $color->name_ar : $color->name_en}}
                <?php //} ?>
              </td>
              <td> <div 
                style="width:25px;height:25px;background-color:{{$color->color}};" 
                class='border m-5'> </div> 
              </td>
              ...
              <td>
                ...
                deleted='Course'
                sendId='{{$color->id}}'
                href="{{ aurl('/') }}/colors/delete/{{$color->id}}">
                <i class='fa fa-trash'></i></a>
              </td>

              <td>
                <a class='color_edit btn btn-info btn-sm' href="/admin/colors/{{$color->id}}/edit"><i class='fa fa-edit'></i></a>
              </td>
            </tr>
          <?php } ?>
        </tbody>
        ...
        <tfoot>
          ...
          <th></th>
          <th>
            <input class='tablefilter' type="text" id=colorname_search placeholder="{{trans('admin.name')}}" />
          </th>
          <th></th>
          <th></th>
          <th></th>
...
<script>
  ...
  $(document).ready( function () {
    ...
    $('#colorname_search').on( 'keyup', function () {
      table.columns(2)
        .search( this.value )
        .draw();
    });
